...
**The Postfix release origin, signature-verified.** The lookaside cache was the original choice and had to be abandoned: it only holds what Fedora packaged, and the requirement is every release Postfix publishes.

`ftp.porcupine.org` is that origin — Wietse Venema's own host, and the spec's `Source0`. Every mirror is a copy of it, and a copy can lag a release by a day or more. The tree is served over both FTP and HTTP, but only FTP answers a directory listing: `official/` over HTTP returns `403`, and the host has no TLS at all. FTP therefore carries both jobs, listing all 400 releases and serving each tarball next to its signatures. The `ftp://ftp.porcupine.org/mirrors/project-history/postfix/` archive linked from the same index page is deeper, reaching back to 1999 and including experimental tarballs, but it stops at 3.8.3 from November 2023 and is not a release channel.

No mirror sits behind it. A mirror is one more host trusted to serve build input, and it buys nothing the origin does not already provide. Mirrors also lag: `ftp.netclusive.de` was still 11 hours behind on 3.11.7. The sweep bears out the rest. `de.postfix.org` does not resolve. `cdn.postfix.johnriley.me`, listed as a CDN mirror, redirects to a LinkedIn profile. Several answer `200` with an HTML landing page in place of the file — `ftp.heikorichter.name` serves a 6 KB HTML page for a `.tar.gz` request, which looks like success to anything checking only the status code. When the origin is unreachable the run fails loudly instead of falling back to a copy.

Trust is not placed in the transport. Every release carries `.sig`/`.gpg1`/`.gpg2` signatures from Wietse Venema; the key is committed at `build/postfix-release-key.asc` and its fingerprint `622C7C012254C186677469C50C0B590E80CA15A7` pinned in `build/build.sh`. Verified working on `postfix-3.10.9.tar.gz` — a release Fedora never packaged — with a good signature.

The pin is anchored outside the download path, so the key is not taken from the same place as the tarball on faith: that key signs `postfix-3.11.6.tar.gz`, and those bytes are byte-identical to Fedora's independently recorded lookaside copy (confirmed by `cmp`). Where Fedora did package the version being built, the `sources` SHA-512 is checked as a second anchor. `pflogsumm-1.1.6.tar.gz` (the spec's `Source53`) still comes from the lookaside, where it is version-independent of Postfix.
